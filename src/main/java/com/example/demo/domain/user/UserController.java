...
public class UserController {
  ...
  @GetMapping("/{id}")
  @PreAuthorize("@userPermissionEvaluator.isUserRequestingOwnResource(authentication.principal.user,#id) || (hasAuthority('DEFAULT') && hasAuthority('USER_MODIFY') && hasAuthority('USER_DELETE'))")
  public ResponseEntity<UserDTO> retrieveById(@PathVariable UUID id) {
    User user = userService.findById(id);
    return new ResponseEntity<>(userMapper.toDTO(user), HttpStatus.OK);
  }

    @Operation(summary = "Get all users", description = "Minimum required role: 'ADMIN'. Minimum required Authority: 'DEFAULT'")
    @PreAuthorize("hasAuthority('DEFAULT') && hasAuthority('USER_MODIFY') && hasAuthority('USER_DELETE')")
    @GetMapping({"", "/"})
  ...
  @Operation(
          ...
          description = "Minimum required role: 'ADMIN'. Minimum required Authority: 'USER_MODIFY'"
  )
  ...
  @Operation(
          ...
          description = "Minimum required role: 'ADMIN'. Minimum required Authority: 'USER_DELETE'"
  )
  @DeleteMapping("/{id}")
